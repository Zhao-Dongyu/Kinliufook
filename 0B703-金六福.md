...
### 1.2运行环境(Run-Time Environment):目标平台上为运行嵌入式软件而必须的的软件环境。

依赖库：python官方和第三方库；OpenCV
中间件：MQTT协议
操作系统

### 1.3编程环境(Programming Environment):为完成嵌入式软件开发而必须的软硬件工具。

#### 1.3.1文本编辑器：vi/vim，gedit,nano
...
是Linux下的一个纯文本编辑器,但你也可以把它用来当成是一个集成开发环境 (IDE), 它会根据不同的语言高亮显现关键字和标识符。


#### 1.3.2交叉编译工具：gcc-linaro-arm-linux-gnueabihf-raspbian-x64  
