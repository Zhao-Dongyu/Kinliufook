...
- 安装配置系统内核包
	...
	如果想把内核镜像做成一个deb包来用，那么必须用安装这个包了。Deb是Debian软件包格式，文件扩展名为.deb，deb包是Unixar的标准归档，将包文件信息以及包内容，经过gzip和tar打包而成，处理deb包的经典程序是dpkg，经常是通过apt来运作。

- 安装配置内核menuconfig的辅助工具

	`sudo apt-get install libncurses5-dev`

	为了能够 make menuconfig
...
显示`configuration written to .config`
