...
Linux系统,我的为虚拟机安装 **ubuntu 16.04 LTS** 系统
...
cd至源代码，执行命令`make ARCH=arm64 CROSS_COMPILE=aarch64-linux-gnu- bcmrpi3_defconfig`

这个过程很快，结果显示`configuration written to .config`

bcmrpi3_defconfig 这个文件位于/Home/Desktop/linux/arch/arm64/configs
...
跑了34分钟电脑烫得严重，庆幸当时配虚拟机还算是给了4个核而不是1个，否则需要更久。

编译好的内核位于/Home/Desktop/linux/arch/arm64/boot,有个image文件，Type为DOS/Windows executable (application/x-ms-dos-executable)，大小为**9.5 MB (9,525,248 bytes)**
...
![config](https://github.com/WolffyAndy/Kinliufook/blob/zhaodongyu/img/config.png)

在menuconfig中对内核配置中的选项进行裁剪，点空格，进行裁剪。
...
-  [ * ] Networking support  ---> <M>   NFC subsystem support  ---> 

	尽管后期有可能用NFC，但是打算的是用模块，用的是串口通信，故而NFC协议估计也用不到。

-  Device Drivers  ---> SPI support 

	用不到SPI
	（后来在编译内核时会遇到以下错误，故而没有裁剪）
...
进行编译内核，这次生成的内核大小为4.3 MB (4,329,472 bytes)，相比之前的9.5 MB少了一半多。

sudo fdisk -l 2018-11-13-raspbian-stretch.img
...
2018-11-13-raspbian-stretch.img1       8192   98045   89854 43.9M  c W95 FAT32 (
2018-11-13-raspbian-stretch.img2      98304 6651903 6553600  3.1G 83 Linux

这个命令的意思就是查看当前这个镜像文件的img区块位置，里面的起点，末尾，扇区的值需要留心观察，等会儿我们会用于计算挂载的地址偏移量。
...
offset ：4194304 = 512 * 8192，sizelimit：46005248 = 512 * 89854

到此，系统原生自带的img镜像就全部挂载到了我们的Ubuntu系统中的/mnt目录下了。

复制 Linux 内核以及设备树到启动分区中：
sudo cp Desktop/linux/arch/arm64/boot/Image /mnt/boot/kernei8.img
sudo cp Desktop/linux/arch/arm64/boot/dts/broadcom/bcm2710-rpi-3-b.dtb /mnt/boot/
...
最后的收尾工作，就是将img烧写到SD卡中，然后，上电，启动。
...
我使用sudo fdisk –l命令查看当前的文件系统的使用情况。需要注意的是这需要root权限。
...
要建立一个新的逻辑分区，需要先建立一个扩展分区

输入e,建立拓展分区，选择默认值，扩展分区出来的是3MB
