...
| 型号 | 主要技术指标 | 成本（元） | 其它 |
| :---: | :--- | :--- | :--- |
| 树莓派 | 处理器 芯片组：Broadcom BCM2837 cortex-A53 64bit SoCat 1.2GHz;内存：1GB SDRAM@400MHz; 无线网络：802.11b/b/n wireless LAN; 蓝牙：Bluetooth 4.1/BLE; USB：4*USB Ports; 电源要求：5V/2.5A; 工作环境温度:0-50℃ | 220 | 采用 |
| Arduino | 8-bit ATmega328处理器; 时钟频率:16MHz; 电源要求:5V/2.5A; SRAM:2KB EEPROM:1KB  | 40 | 不采用 |
| 结论 | Arduino适合做一些与电路、传感器等相关而不需要太多复杂计算的项目，树莓派是一款超小体积的计算机，利用ARM作为处理器，还具有板载内存、HDMI接口、USB接口、音频接口、网络接口等。虽然成本差距很大，但是arduino的运算能力以及兼容性不足以支撑此次项目完成，所以最终我们选择树莓派作为主要硬件开发板。 |
